...
plot_ts2 <- function (x = NULL, y, label_x = "", label_y = "", color = "black", size=1) 
{
  y <- as.vector(y)
  if (is.null(x)) 
    x <- 1:length(y)
...
grf <- plot_ts2(x = data_m$x, y = data_m$temperature, size=0.25) + font
...
grf <- grf + annotate(geom="text", family = fancy_font, x=as.Date('2028-01-01'), y=-1.1, label="A", color="black")
grf <- grf + annotate(geom="text", family = fancy_font, x=as.Date('2028-01-01'), y=-0.25, label="B", color="black")
grf <- grf + annotate(geom="text", family = fancy_font, x=as.Date('2028-01-01'), y= 0.6, label="C", color="black")
grf <- grf + annotate(geom="text", family = fancy_font, x=as.Date('1940-01-01'), y=-2, label="(a)", color="black")
grfB <- grf


grf <- plot_ts2(x = data$x, y = data$temperature, size=0.25) + font
grf <- grf + scale_x_date(breaks = "10 years",  date_labels = "%Y",  limits = c(as.Date("1850-01-01"), as.Date("2030-01-01")))
grf <- grf + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) 
grf <- grf + geom_hline(yintercept = mybin$q[2], col="black", size = 0.5, linetype="dashed")
grf <- grf + geom_hline(yintercept = mybin$q[3], col="black", size = 0.5, linetype="dashed")
...
grf <- grf + annotate(geom="text", family = fancy_font, x=as.Date('2028-01-01'), y=-0.25, label="B", color="black")
grf <- grf + annotate(geom="text", family = fancy_font, x=as.Date('2028-01-01'), y= 0.6, label="C", color="black")
grf <- grf + annotate(geom="text", family = fancy_font, x=as.Date('1940-01-01'), y=-2, label="(b)", color="black")
grfA <- grf


mypng(file="figures/chap5_preprocessing.png", width = 1280, height = 480) #144 #720*1.75
gridExtra::grid.arrange(grfB, grfA, layout_matrix = matrix(c(1,2), byrow = TRUE, ncol = 2))
dev.off()  
