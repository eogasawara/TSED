...
grfD <- function(data, detection, event) {
  ...
  grf <- grf + font 
  grf <- grf + annotate(geom="text", family = fancy_font, x=11, y=16, label="E[1]", color="red", parse=TRUE)
  grf <- grf + annotate(geom="text", family = fancy_font, x=20, y=6, label="E[2]", color="red", parse=TRUE)
  grf <- grf + annotate(geom="text", family = fancy_font, x=12, y=14, label="B", color="blue", parse=TRUE)
  grf <- grf + annotate(geom="text", family = fancy_font, x=21, y=6, label="B", color="blue", parse=TRUE)
  grf <- grf + annotate(geom="text", family = fancy_font, x=10, y=16, label="A", color="blue", parse=TRUE)
