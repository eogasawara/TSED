...
#prob_positive <- c(0.7, 0.9, 0.8, 0.6, 0.55, 0.5, 0.54, 0.53, 0.51, 0.4)
...
grf <- grf + annotate(geom="text", family = fancy_font, x=0.5, y=0.75, label="ROC curve", color="blue")
grf <- grf + annotate(geom="text", family = fancy_font, x=0.15, y=0.9, label="perfect performance", color="darkgreen")
...
grf <- grf + annotate(geom="text", family = fancy_font, x=0.55, y=0.4, label="random performance", color="red")
