...
grf <- har_plot(model, a, detection, event, colors=c("green", "blue", "red", "purple"))
...
grf <- grf + annotate(geom="text", family = fancy_font, x=11, y=15, label="1", color="darkgreen")
grf <- grf + annotate(geom="text", family = fancy_font, x=15, y=8, label="0", color="red")
grf <- grf + annotate(geom="text", family = fancy_font, x=21, y=8, label="0", color="red")
grf <- grf + annotate(geom="text", family = fancy_font, x=2.5, y=11, label="traditional", color="black")
...
grf <- grf + annotate(geom="text", family = fancy_font, x=11, y=7, label="1", color="darkgreen")
grf <- grf + annotate(geom="text", family = fancy_font, x=15, y=0, label="0", color="red")
grf <- grf + annotate(geom="text", family = fancy_font, x=21.5, y=5.5, label="0.5", color="darkgreen")
grf <- grf + annotate(geom="text", family = fancy_font, x=2, y=3, label="softed", color="black")

grf <- grf + geom_segment(aes(x = 11, y = 16, xend = 11, yend = 24), col="green", linewidth = 0.125, linetype="dotdash")
grf <- grf + geom_segment(aes(x = 20, y = 10, xend = 20, yend = 14), col="blue", linewidth = 0.125, linetype="dotdash")

grf <- grf + geom_segment(aes(x = 11, y = 8, xend = 11, yend = 10), col="green", linewidth = 0.125, linetype="dotdash")
grf <- grf + geom_segment(aes(x = 20, y = 2, xend = 20, yend = 10), col="blue", linewidth = 0.125, linetype="dotdash")


grf <- grf + font 
#grf <- grf + annotate(geom="text", family = fancy_font, x=9, y=17.75, label="Method B", color="blue", , angle="90", parse=FALSE)
#grf <- grf + annotate(geom="text", family = fancy_font, x=13.25, y=15, label="Method A", color="red", angle="45", parse=FALSE)
plot(grf)
