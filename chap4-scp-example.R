...
grf <- grf + annotate(geom="text", family = fancy_font, x=9, y=16, label="regression for the entire window", color="blue")
grf <- grf + geom_smooth(aes(xb,datab), color = "green", method=lm, se=FALSE, size = 1, linetype = "dashed")
grf <- grf + annotate(geom="text", family = fancy_font, x=5, y=20, label="regression before change point", color="green")
grf <- grf + geom_smooth(aes(xa,dataa), color = "red", method=lm, se=FALSE, size = 1, linetype = "dashed")
grf <- grf + annotate(geom="text", family = fancy_font, x=13, y=20, label="regression after change point", color="red")
#plot(grf)
